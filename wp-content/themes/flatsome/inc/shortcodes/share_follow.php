...
function flatsome_follow($atts, $content = null) {
    ...
    <div class="<?php echo implode(' ', $wrapper_class); ?>" <?php echo $scale;?>>
	    ...
	    <?php }?>
		<?php if($pinterest){?>
		       <a href="<?php echo $pinterest; ?>" target="_blank" rel="noopener noreferrer nofollow"  data-label="Pinterest"  class="<?php echo $style; ?>  pinterest tooltip" title="<?php _e('Follow on Dribble','flatsome') ?>" aria-label="<?php esc_attr_e( 'Follow on Pinterest', 'flatsome' ); ?>"><?php echo get_flatsome_icon('icon-pinterest'); ?>
		       </a>
		<?php }?>
		<?php if($rss){?>
